...
API : https://ftosint.world/api/numleak?key=shiva-m&num=<NUMBER>
...
VERSION = "2.0.0"
API_URL = "https://ftosint.world/api/numleak?key=shiva-m&num={}"
...
def show_result(number, data):
    ...
    print()
    if chain:
        print(box("  ◉  LEAK CHAIN  ", render_chain(chain)))
        print()
    if calltracer:
        print(box("  📞  CALL TRACER  ", render_calltracer(calltracer)))
        print()
    if not chain and not calltracer:
        print(box("  ▸  DATA  ", [fg("31", "No data returned for this number.")]))
        print()
