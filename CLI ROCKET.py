...
└───────────────────────────────────────────────────────────────┘
"""
import sys, time, json, random, hashlib, re, argparse
...
def render(data, mode, cps, json_out=False):
    ...
    for key, label, m in rows:
        ...
        type_text(f"{BOLD}{label:<13}{RESET} : {v}", m, cps)

    if data.get("lore"):
        pref, (op, circle) = data["lore"]
        type_text(f"{BOLD}{'Orig. Alloc':<13}{RESET} : {pref}xxxx  ->  {op} / {circle}  (legacy lore)",
                  "cyber", cps)
    if data.get("mnp"):
        type_text(f"{BOLD}{'MNP Advisory':<13}{RESET} : {data['mnp']}", "matrix", cps * 2)
    if data.get("flags"):
        type_text(f"{BOLD}{'Patterns':<13}{RESET} : " + "  ".join(f"[{f}]" for f in data["flags"]),
                  "cyber", cps)
    if "anomaly" in data:
        type_text(f"{BOLD}{'Anomaly':<13}{RESET} : {data['anomaly']}/99 (deterministic heuristic)",
                  "neon", cps)
    print()
